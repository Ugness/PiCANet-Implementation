...
if __name__ == '__main__':
    ...
    parser.add_argument('--img', help='Directory of your test_image ""folder""', required=True)
    ...
    os.makedirs(os.path.join(args.logdir, 'img'), exist_ok=True)
    os.makedirs(os.path.join(args.logdir, 'mask'), exist_ok=True)
    ...
    for i, batch in enumerate(dataloader):
        ...
        if args.save_dir is not None:
            torchvision.utils.save_image(img, os.path.join(args.save_dir, 'img', '{}.jpg'.format(i)))
            torchvision.utils.save_image(pred, os.path.join(args.save_dir, 'mask', '{}.jpg'.format(i)))
    writer.close()
